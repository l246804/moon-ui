...
import { fromEntries, getOwnSymbols, hasOwn, isObject, toEntries, unique } from '@/utils'

const wrapperKey = Symbol('propWrapperKey')
export type PropWrapper<T> = { [wrapperKey]: T }

export const propKey = '__moonPropsReservedKeys'

type ResolveProp<T> = ExtractPropTypes<{ key: { type: T; required: true } }>['key']
...
>(option: BuildPropOption<T, D, R, V, C>, key?: string): BuildPropReturn<T, D, R, V, C> {
    ...
    values || validator
      ? (val: unknown) => {
          ...
          if (!valid && allowedValues.length > 0) {
            const allowValuesText = unique(allowedValues)
  ...
  const prop: any = {
    type: isObject(type) && getOwnSymbols(type).includes(wrapperKey) ? type[wrapperKey] : type,
    required: !!required,
...
>(
  ...
  fromEntries(toEntries(props).map(([key, option]) => [key, buildProp(option as any, key)])) as unknown as {
